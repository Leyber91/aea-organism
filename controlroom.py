...
def state() -> dict:
    ...
    return {
        ...
        "life": {"alive_since": hb.get("alive_since"), "wakes": hb.get("boot_count", 0),
                 ...
                 "recent": hb.get("history", [])[-8:], "log": tail(os.path.join(HERE, "live.log"), 6)},
        ...
        "trust": caps,
        "brief": tail(os.path.join(HERE, "brief_output.md"), 24),
    }
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _do(self, req: dict) -> dict:
        ...
        cmd = req.get("cmd")
        if cmd == "brief":
            r = subprocess.run([_sys.executable, "brief.py"], cwd=HERE, capture_output=True,
                               text=True, timeout=300)
            tail = "\n".join((r.stdout or r.stderr or "").strip().splitlines()[-10:])
            return {"ok": r.returncode == 0, "out": tail}
        if cmd == "consolidate":
            r = subprocess.run([_sys.executable, "consolidate.py", "--limit", "2"], cwd=HERE,
        ...
        if cmd == "play":     # PRESS PLAY: start the continuous life, detached (survives this request)
            ...
            logf = open(os.path.join(HERE, "live.log"), "a", encoding="utf-8")
            p = subprocess.Popen([_sys.executable, "live.py"], cwd=HERE, stdout=logf, stderr=logf,
        ...
        if cmd == "tick":     # one bounded tick NOW (blocking; brief can take ~1 min)
            r = subprocess.run([_sys.executable, "live.py", "--interval", "1", "--ticks", "1"],
                               cwd=HERE, capture_output=True, text=True, timeout=420)
